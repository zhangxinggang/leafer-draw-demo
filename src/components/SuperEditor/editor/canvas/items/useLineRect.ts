...
const useLineRect = () => {
  ...
  const addLineRect = ({ event }: IAddRectLineProps) => {
    ...
    console.log(target, preLineRect.current, 'alsjfdklaf');
    if (!preLineRect.current || target.className !== CmpType[CmpType.RectLine]) return;
