 ...
 */
export function pathData2CommandObject(path: IPathCommandData): IPathCommandObject[] {
...
export function point2PathData(points: IPoint[]) {
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    if (type === 'end') {
      ...
      if (prev.x2 === undefined && prev.y2 === undefined && x1 === undefined && y1 === undefined) {
        pathData.push(L, x, y);
      } else if (
        prev.x2 !== undefined &&
        prev.y2 !== undefined &&
 ...
 * @return {*}
 */
export function calculateEuclideanDistance(x: number, y: number, x1: number, y1: number) {
